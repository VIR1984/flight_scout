        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    if partner_link:
                        logger.info(f"✅ Partner link generated: {partner_link[:60]}...")
                        return partner_link
                logger.warning(f"⚠️ Travelpayouts API error ({resp.status}): {await resp.text()}")
                return clean_link
    except asyncio.TimeoutError:
        logger.error("❌ Timeout converting link to partner")
        return clean_link
