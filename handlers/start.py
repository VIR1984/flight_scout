...
def validate_route(text: str) -> tuple:
    ...
    # Normalize city names that might be entered differently
    ...
    if origin != "везде":
        ...
        if not orig_iata:
            await message.answer(f"❌ Не знаю город отправления: {origin}\n"
                                 f"Попробуйте ещё раз.", reply_markup=CANCEL_KB)
            return
...
async def process_depart_date(message: Message, state: FSMContext):
    ...
    # For "Everywhere" searches, skip the return ticket question and go to passengers
    ...
    # For standard searches, ask about return ticket
    kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await message.answer(
        ...
        f"🔄 <b>Шаг 3 из 5:</b> Нужен ли обратный билет?",
        parse_mode="HTML",
        reply_markup=kb
    )
...
async def confirm_search(callback: CallbackQuery, state: FSMContext):
    ...
    if not all_flights:
        ...
        route = f"{origin_iata}{d1}{destinations[0]}{d2}{data.get('passenger_code', '1')}"  # Используем код пассажиров
    ...
    # === ИСПРАВЛЕНО: ВСЕГДА генерируем ссылку через generate_booking_link() с правильными датами и кодом пассажиров ===
    booking_link = generate_booking_link(
        flight=top_flight,
        origin=origin_iata,
        dest=dest_iata,
        depart_date=data["depart_date"],
        passengers_code=data.get("passengers_code", "1"), # <-- Передаём код пассажиров
...
async def handle_flight_request(message: Message):
    ...
    if not all_flights:
        ...
        route = f"{origin_iata}{d1}{dest_iata}{d2}{passengers_code}" # Используем код пассажиров из ручного ввода
        marker = os.getenv("TRAFFIC_SOURCE", "").strip()
        link = f"https://www.aviasales.ru/search/{route}"
        if marker:
            link = add_marker_to_url(link, marker)
    ...
    booking_link = generate_booking_link(
        ...
        dest=dest_iata,
        depart_date=depart_date,
        passengers_code=passengers_code, # <-- Передаём код пассажиров из ручного ввода
