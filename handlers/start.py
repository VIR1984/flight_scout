...
def _update_passengers_in_link(link: str, passengers_code: str) -> str:
    ...
    Примеры:
    • Прямой рейс:   /search/MOW1003IST1   → /search/MOW1003IST211   (для "211")
    ...
    # Удаляем параметры запроса из маршрута, если они есть внутри пути
    if '?' in route_part:
    ...
    if is_relative:
        ...
        return urlunparse((
            ...
            new_path,
            parsed.params,
            query or parsed.query,
            fragment or parsed.fragment
        ))
