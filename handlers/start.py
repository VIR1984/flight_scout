...
def validate_route(text: str) -> tuple:
    ...
    # Разбиваем ТОЛЬКО по разделителю с пробелами вокруг (игнорируем дефисы внутри названий)
    if re.search(r'\s+[-→—>]+\s+', text):
        parts = re.split(r'\s+[-→—>]+\s+', text, maxsplit=1)
    # Если нет пробелов — пробуем другие разделители
    elif any(sym in text for sym in ['→', '—', '>']):
        parts = re.split(r'[→—>]+', text, maxsplit=1)
    else:
        # Просто разбиваем по первому пробелу
...
async def show_help(callback: CallbackQuery):
    help_text = (
        ...
        "• Можно писать «Везде» вместо города вылета — поиск из всех городов России\n"
...
async def confirm_search(callback: CallbackQuery, state: FSMContext):
    ...
    await redis_client.set_search_cache(cache_id, {
        "flights": all_flights,
        "dest_iata": dest_iata,
        "is_roundtrip": data.get("need_return", False),  # Сохраняем как is_roundtrip для кэша
