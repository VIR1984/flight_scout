...
from services.flight_search import (
    ...
    update_passengers_in_link,  # ← КРИТИЧЕСКИ ВАЖНО: импортируем правильную функцию
    add_marker_to_url,
    format_user_date,
...
async def process_everywhere_search(
    ...
    # === КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: используем ЕДИНУЮ функцию из flight_search.py ===
    ...
    if booking_link:
        ...
        booking_link = update_passengers_in_link(booking_link, passengers_code)  # ← ИСПОЛЬЗУЕМ ИМПОРТИРОВАННУЮ ФУНКЦИЮ
