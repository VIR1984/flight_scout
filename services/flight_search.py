...
def test_update_passengers():
    """Тестирование обновления количества пассажиров в ссылках"""
    test_cases = [
        # (исходная ссылка, код пассажиров, ожидаемый результат)
    ...
    print("\n🔍 ТЕСТЫ ОБНОВЛЕНИЯ ПАССАЖИРОВ:")
    for original, code, expected in test_cases:
        result = update_passengers_in_link(original, code)
        status = "✅" if result == expected else "❌"
        print(f"{status} Вход: {original[:30]}... | Код: {code} | Результат: {result == expected}")
        if result != expected:
            print(f"   Ожидалось: {expected}")
            print(f"   Получено:  {result}")
    print()

# Раскомментируйте для запуска тестов при импорте
# if __name__ == "__main__":
 # test_update_passengers()