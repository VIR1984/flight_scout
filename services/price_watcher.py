...
class PriceWatcher:
    ...
    async def start(self):
        ...
        self.running = True
        logger.info("🔄 Запуск наблюдателя за ценами...")
    ...
    async def check_all_watches(self):
        ...
        logger.info(f"🔍 Проверка {len(watch_keys)} отслеживаний...")
        
        # Проверяем по частям, чтобы не нагружать API
        chunk_size = 5
    ...
    async def check_single_watch(self, watch: Dict, key: str):
        ...
        if new_price < current_price:
            ...
            if watch["return_date"]:
                message += f"📅 Возврат: {watch['return_date']}\n"
            
            message += (
                f"\n💰 <b>Старая цена:</b> {current_price} ₽\n"
            ...
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                ...
                [InlineKeyboardButton(text="❌ Больше не следить", 
